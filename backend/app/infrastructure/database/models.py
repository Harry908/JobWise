...
class GenerationModel(Base, TimestampMixin):
    ...
    job_posting: Mapped["JobPostingModel"] = relationship("JobPostingModel", back_populates="generations")
...
class JobApplicationModel(Base, TimestampMixin):
    ...
    # Relationships
    user: Mapped["UserModel"] = relationship("UserModel", back_populates="job_applications")
    job_posting: Mapped["JobPostingModel"] = relationship("JobPostingModel", back_populates="job_applications")
...
def create_performance_indexes():
    ...
    indexes = [
        ...
        # Job search queries  
        "CREATE INDEX IF NOT EXISTS idx_job_title_company ON job_postings (title, company);",
        "CREATE INDEX IF NOT EXISTS idx_job_location_remote_type ON job_postings (location, remote, job_type);",
        
        # Time-based queries
        "CREATE INDEX IF NOT EXISTS idx_generation_completed_time ON generations (completed_at) WHERE completed_at IS NOT NULL;",
        "CREATE INDEX IF NOT EXISTS idx_job_posted_expires ON job_postings (posted_date, expires_date);",
        
        # Full-text search preparation (for PostgreSQL)
        "CREATE INDEX IF NOT EXISTS idx_job_description_gin ON job_postings USING gin(to_tsvector('english', description));",
        "CREATE INDEX IF NOT EXISTS idx_job_requirements_gin ON job_postings USING gin(to_tsvector('english', array_to_string(requirements, ' ')));",
    ]
