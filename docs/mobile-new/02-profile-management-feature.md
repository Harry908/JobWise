...
- `Profile.enhancedSummary` ↔ backend `enhanced_professional_summary`: AI-enriched summary (maps from `enhanced_professional_summary` JSON key); used by generation as a higher-quality source when present.
- `Experience.enhancedDescription` / `Project.enhancedDescription` ↔ backend `enhanced_description`: AI-enriched descriptions; used by generation when available.
- `Experience.achievements` ↔ backend `achievements`: Array of accomplishment strings; displays as "Key Achievements:" section in UI.
- **Date Format**: All date fields (`startDate`, `endDate`) are strings in `yyyy-MM-dd` format. UI uses SettingsService to convert to/from user's preferred display format (US: MM/dd/yyyy, European: dd/MM/yyyy, ISO: yyyy-MM-dd).
- **IDs**: Never include `id` field in POST requests - UUIDs are auto-generated by backend. Only include `id` in PUT/DELETE requests to identify existing items.
...
class Profile {
  ...
  final String? enhancedSummary; // Maps from 'enhanced_professional_summary' in JSON
  final Skills skills;
  ...
  factory Profile.fromJson(Map<String, dynamic> json) {
    return Profile(
      ...
      enhancedSummary: json['enhanced_professional_summary'],
...
```dart
class Experience {
  final String? id; // UUID auto-generated by backend, omit in POST requests
  final String title;
  final String company;
  final String? location;
  final String startDate; // Format: 'yyyy-MM-dd' string
  final String? endDate; // Format: 'yyyy-MM-dd' string  
  final bool isCurrent;
  final String? description;
  final String? enhancedDescription;
  final List<String> achievements; // Array of achievement strings
  final List<String> technologies; // Technologies used (optional)
  ...
  factory Experience.fromJson(Map<String, dynamic> json) {
    return Experience(
      ...
      location: json['location'],
      startDate: json['start_date'], // Already a string 'yyyy-MM-dd'
      endDate: json['end_date'], // Already a string 'yyyy-MM-dd' or null
      isCurrent: json['is_current'] ?? false,
      description: json['description'],
      enhancedDescription: json['enhanced_description'],
      achievements: (json['achievements'] as List?)?.map((e) => e as String).toList() ?? [],
  ...
  Map<String, dynamic> toJson() {
    return {
      if (id != null) 'id': id, // Only include in PUT/DELETE, never in POST
      'title': title,
      'company': company,
      'location': location,
      'start_date': startDate, // Already formatted as 'yyyy-MM-dd'
      'end_date': endDate, // Already formatted as 'yyyy-MM-dd' or null
...
// Note: UI uses SettingsService for date formatting:
// - Display dates using user's preferred format (MM/dd/yyyy, dd/MM/yyyy, or yyyy-MM-dd)
// - Convert to/from API format (yyyy-MM-dd) when saving/loading
```
...
class Education {
  ...
  factory Education.fromJson(Map<String, dynamic> json) {
    return Education(
      ...
      honors: (json['honors'] as List?)?.map((e) => e as String).toList() ?? [],
    );
  }
...
class Project {
  ...
  factory Project.fromJson(Map<String, dynamic> json) {
    return Project(
      ...
      technologies: (json['technologies'] as List?)?.map((e) => e as String).toList() ?? [],
      url: json['url'],
      startDate: json['start_date'], // Already a string 'yyyy-MM-dd' or null
      endDate: json['end_date'], // Already a string 'yyyy-MM-dd' or null
    );
  }
  ...
  Map<String, dynamic> toJson() {
    return {
      ...
      'technologies': technologies,
      'url': url,
      'start_date': startDate, // Already formatted as 'yyyy-MM-dd' or null
      'end_date': endDate, // Already formatted as 'yyyy-MM-dd' or null
    };
  }
}
...
**File**: `lib/widgets/experience_card.dart` (integrated in `profile_view_screen.dart`)

**Features**:
- Displays experience with company, title, dates
- **"Key Achievements:" header** with bold titleSmall styling
- Bullet-pointed achievements list with • prefix
- Enhanced description in green-highlighted container (when available)
- Collapsible original description under ExpansionTile
- Edit icon for quick editing
- Calendar icons for date fields in edit dialogs

**Date Format Integration**:
```dart
// In edit dialogs:
final SettingsService _settingsService = SettingsService();
String _dateFormat = 'MM/dd/yyyy'; // User's preference
...
// On save, convert back to API format
...
**Status**: ✅ Fully Implemented
**Screens**: 3 (View with tabs, Edit, Settings)
**API Endpoints**: 24 endpoints
**Dependencies**: dio, flutter_riverpod, intl, flutter_secure_storage
**Last Updated**: December 1, 2025

**Recent Implementation Updates (Dec 1, 2025)**:
- ✅ **Achievements Display**: Added "Key Achievements:" header in experience cards with bold styling
- ✅ **Achievements Editing**: Full CRUD capability in experience edit dialog with multi-line TextField
- ✅ **Date Pickers**: Calendar UI for all date fields (experiences, education, projects) with GestureDetector + AbsorbPointer pattern
- ✅ **Date Format Integration**: Integrated with SettingsService for user-preferred date formats (US/European/ISO)
  - Display: Converts yyyy-MM-dd API format to user's preferred format using `toDisplayFormat()`
  - Input: Uses `formatDateToDisplay()` for calendar picker selections
  - Save: Converts display format back to yyyy-MM-dd using `toApiFormat()`
- ✅ **Enhanced Professional Summary**: Fixed persistence by updating backend schema and service layer
- ✅ **Tabbed Navigation**: Profile View separated into "Profile" and "AI Preferences" tabs with swipe support
- ✅ **Field Naming Consistency**: Verified `enhanced_professional_summary` naming across frontend/backend
