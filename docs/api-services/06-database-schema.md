...
- **All primary keys are UUIDs** automatically generated by the backend
- **Clients should NEVER provide IDs** when creating resources (POST requests)
- **IDs are required only for updates/deletes** (PUT/DELETE requests) to identify existing resources
- **No ID conflicts possible** - each user's resources are isolated by UUID namespace
