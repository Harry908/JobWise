...
**Note**: Do NOT include an `id` field in the request. IDs are automatically generated by the backend as UUIDs.
...
**Never include IDs in POST requests**:
- ✅ IDs are automatically generated as UUIDs by the backend
- ✅ Each job gets a unique, globally-unique identifier
- ✅ No risk of ID conflicts between users
- ❌ Do not provide `id` field when creating jobs
- ✅ IDs are only required for PUT/DELETE operations (in URL path)

### 2. Text Parsing Quality
