...
1. **Writing Style Extraction** - Analyze sample cover letter to extract style preferences (stored in database)
...
### Purpose
Extract writing style characteristics from user's sample cover letter and store in database for reuse across profile enhancements.

### Trigger
- Automatically triggered when user uploads a cover letter sample
- Can be manually re-triggered via API endpoint
- Called by Profile Enhancement workflow if no cached style exists

### Input
- Sample cover letter text (from `sample_documents` table)

### Output
- `WritingStyle` object stored in `sample_documents.extracted_writing_style` field
- Status tracking in `style_extraction_status`, `style_extraction_timestamp`, etc.
...
└──────────────────────────────────────────────────────────────┘
                           ↓
┌──────────────────────────────────────────────────────────────┐
│ STEP 5: Store WritingStyle in Database                      │
├──────────────────────────────────────────────────────────────┤
│ UPDATE sample_documents SET                                  │
│   extracted_writing_style = ?,  -- JSON string              │
│   style_extraction_status = 'completed',                    │
│   style_extraction_model = 'llama-3.1-8b-instant',         │
│   style_extraction_timestamp = NOW(),                       │
│   style_extraction_confidence = 0.92                        │
│ WHERE id = ? AND document_type = 'cover_letter'             │
│                                                              │
│ Cached for future use in profile enhancement                │
└──────────────────────────────────────────────────────────────┘
```
